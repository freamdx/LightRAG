...
if not pipmaster.is_installed("asyncpg"):
    pipmaster.install("asyncpg")
...
class ADBMYSQL:
    def __init__(self, config: dict[str, Any], **kwargs: Any):
        ...
        self.user = config["adb_user"]
        self.password = config["adb_password"]
        self.database = config["adb_database"]
    ...
    async def initialize(self):
        async with get_data_init_lock():
            ...
            async with self.db.pool.acquire() as connection:
                ...
                await ADBMYSQL.configure_age_extension(connection)
